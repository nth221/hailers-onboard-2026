# ==========================================================
#  v22.py — 범주형 임베딩
#  가설: 원-핫으로 펼쳐진 희소 컬럼이 성능을 제약한다.
#        임베딩으로 대체하면 입력 차원이 축소되고 표현이 학습되어 개선될 것.
#  고정: 왜도보정+서열형확장, 구조 32→8, K-fold 분할
# ==========================================================
...
def run(lr, hidden=(32, 8), p=0.1, seed=42, epochs=3000, patience=200):
    ...
    for tr_idx, va_idx in kf.split(X_num):
        ...
        Xn_tr, Xc_tr, y_tr = Xn_tr.drop(index=drop), Xc_tr.drop(index=drop), y_tr.drop(index=drop)
        ...
        model = HousePriceEmbedModel(Xn_tr_t.shape[1], cards, hidden=hidden, p=p)
    ...
    return np.sqrt(((oof - y.values) ** 2).mean()), torch.stack(test_preds).mean(dim=0), model.input_dim
...
best_score, best_test = None, None
for lr in [0.002, 0.005]:                    # v17 교훈: 표현 바뀌면 lr 재탐색
    score, te, in_dim = run(lr)
    print(f">>> lr={lr} | 입력차원 {in_dim} | OOF {score:.4f}")
    if best_score is None or score < best_score:
        best_score, best_test = score, te

print("=" * 60)
print(f"최고 OOF {best_score:.4f}   (원-핫 기준 0.1278 / Kaggle 0.13343)")
